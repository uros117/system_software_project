...
// https://www.programmersought.com/article/90525699763/
// Non-canonical mode

volatile uint16_t term_in = 0;
uint16_t term_out = 0;
...
int term_init() {
    ...
    terminal_settings.c_cc[VTIME] = 5;
    ...
    pthread_create( &read_loop_thread, NULL, read_loop, NULL);
...
void term_loop() {
    ...
    if(rec_flag) {// rec_flag
        // READ
        signal_interrupt(INT_KEYBRD);
        rec_flag = 0;
    }
    /*if(write_flag) {
        putc(term_out, stdout);
...
void term_deinit() {
    read_loop_run = 0;
    pthread_cancel( read_loop_thread);
}
